...
// primary: #00B4D8 = 0x00B4D8
// secondary: #F7941D = 0xF7941D
// background: #020617 = 0x020617
const defaultColors = {
  roadColor: 0x080808,
  islandColor: 0x0a0a0a,
  background: 0x020617, // bg-background
  shoulderLines: 0x131318,
  brokenLines: 0x131318,
  leftCars: [0x00B4D8, 0x00B4D8, 0x00B4D8], // primary
  rightCars: [0xF7941D, 0xF7941D, 0xF7941D], // secondary
  sticks: 0x00B4D8 // primary
};
...
const Hyperspeed = ({
  ...
  useEffect(() => {
    ...
    const initApp = () => {
      if (!container || container.offsetWidth === 0 || container.offsetHeight === 0) {
        ...
        console.log('[Hyperspeed] Waiting for container dimensions...', {
      ...
      console.log('[Hyperspeed] Initializing with dimensions:', {
        width: container.offsetWidth,
        height: container.offsetHeight
      });
    ...
    class App {
      ...
      constructor(container: HTMLElement, options: any = {}) {
        ...
        // Prevent zero-size initialization
        if (width <= 0) width = window.innerWidth || 1920;
        if (height <= 0) height = window.innerHeight || 1080;
        
        // Check WebGL support
        const testCanvas = document.createElement('canvas');
        const gl = testCanvas.getContext('webgl') || testCanvas.getContext('experimental-webgl');
        if (!gl) {
          console.error('[Hyperspeed] WebGL is not supported in this browser');
          throw new Error('WebGL not supported');
        }
        console.log('[Hyperspeed] WebGL supported, creating renderer...', { width, height });
        ...
        // Temporary: make canvas visible for debugging
        // canvas.style.border = '2px solid red';
        
        console.log('[Hyperspeed] Canvas created and styled:', {
          width: canvas.width,
        ...
        console.log('[Hyperspeed] Canvas appended to container:', container.contains(canvas));
        ...
        console.log('[Hyperspeed] Camera and scene created:', {
      ...
      // Add a method to check if rendering is working
      checkRenderStatus() {
        const canvas = this.renderer.domElement;
        const size = new THREE.Vector2();
        this.renderer.getSize(size);
        console.log('[Hyperspeed] Render status:', {
          canvasVisible: canvas.offsetWidth > 0 && canvas.offsetHeight > 0,
          canvasInDOM: document.body.contains(canvas) || this.container.contains(canvas),
          sceneChildren: this.scene.children.length,
          rendererSize: { width: size.x, height: size.y },
          canvasStyle: {
          ...
          cameraPosition: { x: this.camera.position.x, y: this.camera.position.y, z: this.camera.position.z },
          roadExists: !!this.road,
          carLightsExist: !!this.leftCarLights && !!this.rightCarLights,
          sticksExist: !!this.leftSticks
        });
      }
    }
    ...
    const roadMarkings_fragment = `
      ...
      float lines = mix(brokenLines, sideLines, uv.x);
      color = mix(color, uBrokenLinesColor, lines);
    `;
    ...
    `;
        ...
        if (!options.distortion) {
          console.error('Invalid distortion type:', effectOptions.distortion);
          return;
        }

        const myApp = new App(container, options);
        appRef.current = myApp;
        console.log('[Hyperspeed] App created, loading assets...');
        myApp.loadAssets().then(() => {
          console.log('[Hyperspeed] Assets loaded, initializing...');
          myApp.init();
          console.log('[Hyperspeed] Initialization complete!');
          // Check render status after a short delay
          setTimeout(() => {
            myApp.checkRenderStatus();
          }, 1000);
        }).catch((error) => {
          console.error('[Hyperspeed] Initialization error:', error);
        });
      } catch (error) {
        console.error('[Hyperspeed] Setup error:', error);
      }
    };
  ...
  return (
    <div 
      id="lights" 
      ref={hyperspeed} 
      className={cn("w-full h-full absolute inset-0 z-0 bg-background", className)}
      style={{ zIndex: 0 }}
    >
      {children}
    </div>
